/**
 * Align Payouts daily totals to client payout schedule (CreatedOn = payout date).
 * Payouts table ONLY — no Rewards, wallets, or other tables touched.
 *
 * Usage:
 *   $env:DB_NAME="test"; npx ts-node -r tsconfig-paths/register -r dotenv/config scripts/align-payouts-payout-schedule.ts
 *   ... --execute
 */
...
async function findTemplatePayouts(beforeDate: string): Promise<PayoutRow[]> {
  for (let i = PAYOUT_DATES.indexOf(beforeDate) - 1; i >= 0; i--) {
    const rows = await loadPayoutsForDate(PAYOUT_DATES[i]);
...
async function main() {
  ...
  console.log(`Scope: Payouts only, ${WINDOW_START} – ${WINDOW_END}`);
  ...
  for (const dateStr of PAYOUT_DATES) {
    ...
    if (rows.length === 0) {
      const template = await findTemplatePayouts(dateStr);
      if (!template.length) {
  ...
  if (execute) {
    ...
    for (const r of verify) {
      ...
      if (r.d >= WINDOW_START && r.d <= WINDOW_END) vSum += total;
      const ok =
        client != null && Math.abs(total - client) < 0.00000002 ? "OK" : client != null ? "DIFF" : "—";
